...
* `on_duplicate_behaviour`: By default, the package will replay previously-seen responses when an idempotency key is re-used. Your application may wish to handle this duplication differently - throwing an error, for example. Setting this value to `\Square1\LaravelIdempotency\Enums\DuplicateBehaviour::EXCEPTION` will cause a `DuplicateRequestException` to be thrown in this case. Defaults to `\Square1\LaravelIdempotency\Enums\DuplicateBehaviour::REPLAY`.
...
* `DuplicateRequestException`: By default the package will replay a response when a previous idempotency key is seen again. Changing the config value `on_duplicate_behaviour` to `\Square1\LaravelIdempotency\Enums\DuplicateBehaviour::EXCEPTION` will cause an exception to be thrown instead (useful for applications when a re-sent request is more likely a bug in the client).
